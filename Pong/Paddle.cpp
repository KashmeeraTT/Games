...
void Paddle::updatePlayer(bool isUpPressed, bool isDownPressed, float maxHeigth) {
    if (isUpPressed && shape.getPosition().y > 0) {
        shape.move(0, -playerSpeed);
    }
    if (isDownPressed && shape.getPosition().y < maxHeigth - shape.getSize().y) {
        shape.move(0, playerSpeed);
    }
}

void Paddle::updateAI(float ballY, Constants::AIDifficulty difficulty, float maxHeigth) {
    float aiSpeed = 0.0f;
    float offset = 0.0f;
    switch (difficulty) {
        ...
        case Constants::AIDifficulty::HARD:   aiSpeed = 11.0f; offset = 5.0f;  break;
    }

    float aiCenter = shape.getPosition().y + (Constants::PADDLE_HEIGHT / 2);
    float ballCenter = ballY + (Constants::BALL_SIZE / 2);

    if (aiCenter < ballCenter - offset && shape.getPosition().y < maxHeigth - shape.getSize().y) {
